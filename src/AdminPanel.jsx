...
const createEmptyCar = () => ({ brand: '', name: '', year: new Date().getFullYear(), mileage: 0, fuel: 'Бензин', drive: 'Передний привод', price: '', tone: 'graphite', photos: '', equipment: [...defaultEquipment] })
...
export default function AdminPanel({ token, setToken, onClose, onCarsChanged }) {
  ...
  const startEdit = (car) => { setEditing(car.id); setForm({ ...car, photos: (car.photos || []).join('\n'), equipment: Array.isArray(car.equipment) ? car.equipment : [...defaultEquipment] }); window.scrollTo({ top: 0, behavior: 'smooth' }) }

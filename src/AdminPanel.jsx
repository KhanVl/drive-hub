...
const emptyCar = { brand: '', name: '', year: new Date().getFullYear(), mileage: 0, fuel: 'Бензин', drive: 'Передний привод', price: '', tone: 'graphite', photos: '' }
...
export default function AdminPanel({ token, setToken, onClose, onCarsChanged }) {
  ...
  const uploadPhotos = async (event) => {
    ...
    try {
      ...
      for (const file of files) {
        if (file.size > 8_000_000) throw new Error(`${file.name}: файл больше 8 МБ`)
  ...
  const cancelEdit = () => { setEditing(null); setForm(emptyCar); setError('') }

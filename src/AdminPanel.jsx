...
export default function AdminPanel({ token, setToken, onClose, onCarsChanged }) {
  ...
  const deleteInquiry = async (inquiry) => {
    if (!window.confirm(`Удалить заявку от «${inquiry.name}»? Это действие нельзя отменить.`)) return
    ...
    finally { setDeletingInquiry(null) }
  }
  const logout = async () => {
